...
# Función para comenzar el temporizador en reversa
def comenzar_temporizador():
    ...
    ventana_temporizador(duracion_segundos)

# Función para convertir la duración seleccionada en segundos
...
# Ventana del temporizador
def ventana_temporizador(segundos):
    ventana = tk.Toplevel()
    ventana.title("Temporizador")
    ventana.geometry("300x200")

    etiqueta_temporizador = tk.Label(ventana, text="", font=("Caviar Dreams", 24))
...
# ------------------------------------------------------ MENU ------------------------------------------------------
def menu():
    canvas.create_text(canvas_width / 2, 80, text="Bienvenido!", font=("Mulbena", 100), fill="#1b1b1b")

    # - DURACION -----------------------------------------------------------------------------------------------
    label_duracion = tk.Label(root, text="Duración:", font=("Caviar Dreams", 50))
    ...
    # - INTENSIDAD ---------------------------------------------------------------------------------------------
    label_intensidad = tk.Label(root, text="Intensidad:", font=("Caviar Dreams", 50))
    ...
    # - FRECUENCIA ---------------------------------------------------------------------------------------------
    label_frecuencia = tk.Label(root, text="Frecuencia:", font=("Caviar Dreams", 50))
    ...
    # - COMENZAR -----------------------------------------------------------------------------------------------
    btn_comenzar = tk.Button(root, text="Comenzar", font=("Caviar Dreams", 24), command=comenzar_temporizador)
    btn_comenzar.place(relx=0.5, rely=0.8, anchor=tk.CENTER)
    # ----------------------------------------------------------------------------------------------------------

    # - SALIR --------------------------------------------------------------------------------------------------
    btn_salir = tk.Button(root, text="Salir", font=("Caviar Dreams", 24), command=root.quit)
    btn_salir.place(relx=0.5, rely=0.9, anchor=tk.CENTER)
    # ----------------------------------------------------------------------------------------------------------

    # - TEMPORIZADOR -------------------------------------------------------------------------------------------
    global etiqueta_temporizador
    etiqueta_temporizador = tk.Label(root, text="", font=("Caviar Dreams", 24))
...
root.after(1000, intro_logo, logo_velocidad, logo_mov, logo_duracion)
# ------------------------------------------------------------------------------------------------------------------
